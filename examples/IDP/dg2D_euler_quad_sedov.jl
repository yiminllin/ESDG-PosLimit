...
const N = 3    # N = 2,3,4
const K1D = 30  # K = 10,20,40,80,160
const T = 1.0
const dt0 = 1e+1
const CFL = 0.9
...
const OUTPUTPATH = "/home/yiminlin/Desktop/dg2D_euler_quad_sedov_N=$(N)_K1D=$(K1D)_LBOUNDTYPE_$(LBOUNDTYPE)_IFSHOCK=$(IFSHOCKCAPTURE)"
mkdir(OUTPUTPATH)
...
function rhs_IDP!(U,rhsU,t,dt,prealloc,ops,geom,in_s1)
    f_x,f_y,rholog,betalog,U_low,U_high,F_low,F_high,F_P,L,wspd_arr,λ_arr,λf_arr,dii_arr,L_plot,rhop,epsN,blending = prealloc
    S0xJ_vec,S0yJ_vec,S0r_nnzi,S0r_nnzj,S0s_nnzi,S0s_nnzj,SxJ_db_vec,SyJ_db_vec,Sr_nnzi,Sr_nnzj,Ss_nnzi,Ss_nnzj,MJ_inv,BrJ_halved,BsJ_halved,coeff_arr,S_nnzi,S_nnzj,VDMinv = ops
    mapP,Fmask,x,y = geom
...
for k = 1:K
    for i = 1:Np
        rho,u,v,p = sedov_init(x[i,k],y[i,k])
        U[1,i,k] = rho
        U[2,i,k] = rho*u
        U[3,i,k] = rho*v
...
prealloc = (f_x,f_y,rholog,betalog,U_low,U_high,F_low,F_high,F_P,L,wspd_arr,λ_arr,λf_arr,dii_arr,L_plot,rhop,epsN,blending)
ops      = (S0xJ_vec,  S0yJ_vec,  S0r_nnzi,S0r_nnzj,S0s_nnzi,S0s_nnzj,
            SxJ_db_vec,SyJ_db_vec,Sr_nnzi, Sr_nnzj, Ss_nnzi, Ss_nnzj,
            MJ_inv,BrJ_halved,BsJ_halved,coeff_arr,S_nnzi,S_nnzj,VDMinv)
geom     = (mapP,Fmask,x,y)
...
function construct_vtk_file!(U,Uvtk,pvdfile,xvtk,yvtk,t)
    vtk_grid("$(OUTPUTPATH)/dg2D_euler_quad_sedov_t=$t",xvtk,yvtk) do vtk
        for k = 1:K
            iK = mod1(k,K1D)
            jK = div(k-1,K1D)+1
            for c = 1:Nc
...
x_vtk    = zeros(Float64,(N+1)*K1D,(N+1)*K1D)    # TODO: hardcoded domain size
y_vtk    = zeros(Float64,(N+1)*K1D,(N+1)*K1D)
U_vtk    = zeros(Float64,Nc,(N+1)*K1D,(N+1)*K1D)

construct_vtk_mesh!(x_vtk,y_vtk)
pvd = paraview_collection("$(OUTPUTPATH)/dg2D_euler_quad_sedov.pvd")
