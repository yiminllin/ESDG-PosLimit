...
function rhs_IDP(U,K,N,wq,S,S0,Mlump_inv,T,dt,t,in_s1,is_low_order,LBOUNDTYPE)
    rhoL  = 3.857143
    rhouL = 3.857143*2.629369
    uL    = 2.629369
    pL    = 10.3333
    EL    = pL/(γ-1) + .5*rhoL*(uL^2)
    rhoR  = U[1][end,end]
    rhouR = U[2][end,end]
    uR    = U[2][end,end]/U[1][end,end]
    ER    = U[3][end,end]
...
ax2 = Axis(f2[1,1])
limits!(ax1, -5, 5, 0, 6)
limits!(ax2, -2.5, 2.5, 2, 6)

lw = 3

weno_sol = matread("weno5_shuosher.mat")
l1 = lines!(ax1,weno_sol["x"][:],weno_sol["rho"][:],linestyle=nothing,linewidth=lw,color=:gray25,label="reference")
l1 = lines!(ax2,weno_sol["x"][:],weno_sol["rho"][:],linestyle=nothing,linewidth=lw,color=:gray25,label="reference")
...
LBOUNDTYPEarr = [0.0 0.1 0.5]

# Start N, K loop
for N in Narr
for K in Karr
for LBOUNDTYPE in LBOUNDTYPEarr
...
"""Initial condition"""
# rho_x(x) = (x < xC) ? 3.857143 : 1+.2*sin(5*x)
# u_x(x) = (x < xC) ? 2.629369 : 0.0
# p_x(x) = (x < xC) ? 10.33333 : 1.0
rho_x(x) = (x < xC) ? 3.857143 : 1+.2*sin(5*x)
u_x(x) = (x < xC) ? 2.629369 : 0.0
p_x(x) = (x < xC) ? 10.3333 : 1.0

function exact_sol(x,t)
    return rho_x(x),u_x(x),p_x(x)
end
...
while t < T
    ...
    dt = Inf
    # SSPRK(3,3)
    rhsU,dt = rhs_IDP(U,K,N,wq,S,S0,Mlump_inv,T,dt,t,true,is_low_order,LBOUNDTYPE)
    #rhsU = rhs_high(U,K,N,Mlump_inv,S)
    # rhsU,dt = rhs_IDPlow(U,K,N,Mlump_inv,wq)
    @. resW = U + dt*rhsU
    rhsU,_ = rhs_IDP(resW,K,N,wq,S,S0,Mlump_inv,T,dt,t,false,is_low_order,LBOUNDTYPE)
...
rho = U[1]
if (LBOUNDTYPE == 0.0)
    l1 = lines!(ax1,x[:],rho[:],linestyle=[0.5,1.0],linewidth=lw,color=:seagreen3,label="zeta = $LBOUNDTYPE")
    axislegend(ax1,labelsize=20)
    l1 = lines!(ax2,x[:],rho[:],linestyle=[0.5,1.0],linewidth=lw,color=:seagreen3,label="zeta = $LBOUNDTYPE")
    axislegend(ax2,labelsize=20)
end
if (LBOUNDTYPE == 0.1)
    l1 = lines!(ax1,x[:],rho[:],linestyle=[0.5,1.0],linewidth=lw,color=:royalblue1,label="zeta = $LBOUNDTYPE")
    axislegend(ax1,labelsize=20)
    l1 = lines!(ax2,x[:],rho[:],linestyle=[0.5,1.0],linewidth=lw,color=:royalblue1,label="zeta = $LBOUNDTYPE")
    axislegend(ax2,labelsize=20)
end
if (LBOUNDTYPE == 0.5)
    l1 = lines!(ax1,x[:],rho[:],linestyle=[0.5,1.0],linewidth=lw,color=:darkorange1,label="zeta = $LBOUNDTYPE")
    axislegend(ax1,labelsize=20)
    l1 = lines!(ax2,x[:],rho[:],linestyle=[0.5,1.0],linewidth=lw,color=:darkorange1,label="zeta = $LBOUNDTYPE")
    axislegend(ax2,labelsize=20)
end
...
save("dg1D-euler-shuosher-limited-N=$(Narr[1])-K=$(Karr[1]).png",f1)
save("dg1D-euler-shuosher-limited-N=$(Narr[1])-K=$(Karr[1])-zoom.png",f2)