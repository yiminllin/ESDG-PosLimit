...
const OUTPUTPATH = "/home/yiminlin/Desktop/dg2D_euler_tri_sedov_N=$(N)_K1D=$(K1D)_LBOUNDTYPE_$(LBOUNDTYPE)_IFSHOCK=$(IFSHOCKCAPTURE)"
mkdir(OUTPUTPATH)
...
function rhs_Limited!(U,rhsU,T,dt,prealloc,ops,geom,in_s1,is_low_order)
    f_x,f_y,rholog,betalog,U_low,U_high,F_low,F_high,F_P,L,λ_arr,λf_arr,dii_arr,L_plot,U_hat,indicator,rhop,epsN,blending = prealloc
    Sr,Ss,S0r,S0s,Br,Bs,Minv,Extrap,Pq,Vq,wq,Vq_hat,M,VDMinv = ops
    ...
    if (IFSHOCKCAPTURE)
        @batch for k = 1:K
            @views mul!(epsN[:,k],VDMinv*Pq,rhop[:,k])
        end
        ...
        for j = 2:Np
            for i = 1:j-1
                for c = 1:Nc
                    ...
                    # rhsU[c,i,k] = rhsU[c,i,k] - 1.0*FL_ij
                    # rhsU[c,j,k] = rhsU[c,j,k] + 1.0*FL_ij
                end
            end
        end
...
perm = reference_plotting_triangulation((rq,sq))
points = collect(transpose([xq[:] yq[:]]))
num_subtri = size(perm,1)
cells = [MeshCell(VTKCellTypes.VTK_TRIANGLE, perm[mod1(i,num_subtri),:] .+ Np*div(i-1,num_subtri)) for i = 1:num_subtri*K]

function construct_vtk_file!(U,points,cells,pvdfile,t)
    vtk_grid("$(OUTPUTPATH)/dg2D_euler_tri_sedov_t=$t",points,cells) do vtk
        vtk["rho"]  = U[1,:,:]
        vtk["rhou"] = U[2,:,:]
        vtk["rhov"] = U[3,:,:]
        vtk["E"]    = U[4,:,:]
...
pvd = paraview_collection("$(OUTPUTPATH)/dg2D_euler_tri_sedov.pvd")

const r_ini = 4*3/K1D
function sedov_init(x,y)
    r = sqrt(x^2+y^2)
    if (r < r_ini)
...
for k = 1:K
    for i = 1:Np
        rho,u,v,p = sedov_init(xq[i,k],yq[i,k])
        U[1,i,k] = rho
        U[2,i,k] = rho*u
        U[3,i,k] = rho*v
...
@unpack VDM = rd
VDMinv = inv(VDM)
const Nmodal = size(VDM,2)
const Nnodal = size(VDM,1)
...
Vq_hat = VDM_hat/VDM
prealloc = (f_x,f_y,rholog,betalog,U_low,U_high,F_low,F_high,F_P,L,λ_arr,λf_arr,dii_arr,L_plot,U_hat,indicator,rhop,epsN,blending)
ops      = (Sr,Ss,S0r,S0s,Br,Bs,Minv,E,Pq,Vq,wq,Vq_hat,M,VDMinv)
...
@time while t < T
    ...
    # dt = min(dt0,T-t)
    dt = dt0
    dt = rhs_Limited!(U,rhsU,T,dt,prealloc,ops,geom,true,false)
    # dt = rhs_IDP!(U,rhsU,dt,prealloc,ops,geom,true)
    # rhs_ESDG!(U,rhsU,prealloc,ops,geom)
    # dt = min(CFL*dt,T-t)
    @. resW = U + dt*rhsU
    rhs_Limited!(resW,rhsU,T,dt,prealloc,ops,geom,false,false)
    ...
    global i = i + 1
    if (mod(i,1000) == 1)
        construct_vtk_file!(U,points,cells,pvd,t)
    end
end
