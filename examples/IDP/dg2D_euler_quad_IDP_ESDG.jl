...
function get_lambda_j(N,Np,j,face_idx)
    lambda_j = 1/(Np-1)
    if j in face_idx
        if j == 1 || j == Np || j == N+1 || j == Np-N
            lambda_j = 1/(Np+1)
        else
            lambda_j = 1/Np
        end
...
function rhs_IDP(U,K1D,N,Nfaces,nxJ,nyJ,Minv,Sr,Ss,S0r,S0s,Br,Bs,face_idx,mapM,mapP_x,mapP_y,dt)
    ...
    # limiting parameters on interfaces
    ...
    U_low    = [zeros(Np,K) for i = 1:Nc]
    L       = zeros(Np,Np) # limiting params over interior
    L_P     = sparse(I_arr,J_arr,zeros(size(I_arr)))

    L_plot = zeros(Np,K)
    
    # Calculate low and high order fluxes at interfaces
    for k = 1:K
    ...
    # Loop over interior of each elements, calculate low order approximation
    # Store the limiting solution (only interior part) temporarily in rhsU
    for k = 1:K
        ...
        for i = 1:Np
            for c = 1:Nc
                U_low[c][i,k] += sum(F_low[c][i,:])
                U_low[c][i,k] += sum(F_P_low[c][i+(k-1)*Np,:])
            end
        end
        for c = 1:Nc
            U_low[c][:,k] = U[c][:,k]-1/J*Minv*U_low[c][:,k]
        end 

        # Calculate limiting parameters limiting_param(U_low, P_ij)
        for i = 1:Np
            m_i = J*M[i,i]
            lambda_j = get_lambda_j(N,Np,i,face_idx)
            U_low_i = [U_low[c][i,k] for c = 1:Nc]
            for j = 1:Np
                if i != j 
                    P_ij = [dt/m_i/lambda_j*(F_low[c][i,j]-F_high[c][i,j]) for c = 1:Nc]
        ...
        # Update limiting solution with "interior info"
        for c = 1:Nc
            rhsU[c][:,k] = sum((L.-1).*F_low[c] - L.*F_high[c],dims=2)
        end

        # Plot limiting parameters
        L_plot[:,k] = sum(L,dims=2)
    end

    # Loop over interfaces, calculate limiting params
    for k = 1:K
        for i = 1:Nfp
            fidM = mapM[i,k]
            m_i = J*M[face_idx[i],face_idx[i]]
            lambda_j = get_lambda_j(N,Np,i,face_idx)
            U_low_i = [U_low[c][face_idx[i],k] for c = 1:Nc]
            fidP_x = mapP_x[i,k]
            fidP_y = mapP_y[i,k]

            if fidP_x != 0
                P_ij = [dt/m_i/lambda_j*(F_P_low[c][fidM,fidP_x]-F_P_high[c][fidM,fidP_x]) for c = 1:Nc]
                L_P[fidM,fidP_x] = limiting_param(U_low_i, P_ij)
            end

            if fidP_y != 0
                P_ij = [dt/m_i/lambda_j*(F_P_low[c][fidM,fidP_y]-F_P_high[c][fidM,fidP_y]) for c = 1:Nc]
                L_P[fidM,fidP_y] = limiting_param(U_low_i, P_ij)
            end
        end
    end

    # Symmetrize limiting param on interfaces
    for k = 1:K
        for i = 1:Nfp
            fidM = mapM[i,k]
            fidP_x = mapP_x[i,k]
            ...
            if fidP_x != 0
                l = min(L_P[fidM,fidP_x], L_P[fidP_x,fidM])
                L_P[fidM,fidP_x] = l
                L_P[fidP_x,fidM] = l

                # Update limiting solution with "interface info"
                for c = 1:Nc
                    rhsU[c][fidM] += (l-1)*F_P_low[c][fidM,fidP_x] - l*F_P_high[c][fidM,fidP_x]
                end
            end
            if fidP_y != 0
                l = min(L_P[fidM,fidP_y], L_P[fidP_y,fidM])
                L_P[fidM,fidP_y] = l
                L_P[fidP_y,fidM] = l
                
                # Update limiting solution with "interface info"
                for c = 1:Nc
                    rhsU[c][fidM] += (l-1)*F_P_low[c][fidM,fidP_y] - l*F_P_high[c][fidM,fidP_y]
                end
            end
        end
    end
    ...
    for k = 1:K
        for i = 1:Np
            L_plot[i,k] += sum(L_P[i+(k-1)*Np,:])
            L_plot[i,k] = 1.0 - L_plot[i,k]/get_lambda_j(N,Np,i,face_idx)
        end
    end
...
while t < T
    ...
    dt = min(dt,T-t)
    @. resW = U + dt*rhsU
    rhsU,_ = rhs_IDP(resW,K1D,N,Nfaces,nxJ,nyJ,Minv,Sr,Ss,S0r,S0s,Br,Bs,face_idx,mapM,mapP_x,mapP_y,dt)
    @. resZ = resW+dt*rhsU
    @. resW = 3/4*U+1/4*resZ
    rhsU,_ = rhs_IDP(resW,K1D,N,Nfaces,nxJ,nyJ,Minv,Sr,Ss,S0r,S0s,Br,Bs,face_idx,mapM,mapP_x,mapP_y,dt)
    @. resZ = resW+dt*rhsU
