...
@inline function is_left_boundary(i,xM)
    return (abs(xM+0.5) < TOL) && (i > TOPLEFT)
end

@inline function is_right_boundary(i,xM)
    return (abs(xM-1.0) < TOL) && (i > BOTTOMRIGHT) && (i <= TOPRIGHT)
end

@inline function is_boundary(i,xM)
    return is_left_boundary(i,xM) || is_right_boundary(i,xM)
end
...
@inline function get_consP(UP,U,mapP,Fmask,i,iM,xM,yM,uM,vM,k,tid)
    ...
    if is_left_boundary(i,xM)
        UP[1,tid] = rhoL
        UP[2,tid] = rhoL*uL
        UP[3,tid] = rhoL*vL
        UP[4,tid] = EL
    end
    if is_right_boundary(i,xM)
        UP[1,tid] = rhoR
        UP[2,tid] = rhoR*uR
        UP[3,tid] = rhoR*vR
        UP[4,tid] = ER
...
@inline function get_fluxP(fP,f_x,f_y,mapP,Fmask,i,xM,yM,k,UP,tid)
    ...
    if is_left_boundary(i,xM)
        ...
        fP[1,2,tid],fP[2,2,tid],fP[3,2,tid],fP[4,2,tid] = inviscid_flux_prim(rhoL,uL,vL,pL)
    end
    if is_right_boundary(i,xM)
        fP[1,1,tid],fP[2,1,tid],fP[3,1,tid],fP[4,1,tid],
        fP[1,2,tid],fP[2,2,tid],fP[3,2,tid],fP[4,2,tid] = inviscid_flux_prim(rhoR,uR,vR,pR)
    end
end
...
@inline function get_vP(VUP,VU,mapP,Fmask,i,iM,xM,yM,k,tid) 
    ...
    if is_left_boundary(i,xM)
        VUP[1,tid],VUP[2,tid],VUP[3,tid],VUP[4,tid] = entropyvar(rhoL,rhoL*uL,rhoL*vL,EL)
    end
    if is_right_boundary(i,xM)
        VUP[1,tid],VUP[2,tid],VUP[3,tid],VUP[4,tid] = entropyvar(rhoR,rhoR*uR,rhoR*vR,ER)
    end
end
...
function exact_sol_viscous_shocktube(x,y,t)
...
for k = 1:K
    for i = 1:Np
        U[1,i,k],U[2,i,k],U[3,i,k],U[4,i,k] = exact_sol_viscous_shocktube(x[i,k],y[i,k],0.0)
    end
end
...
#while i < 2
    ...
    #     v_int = U[3,:,:]./U[1,:,:]
    #     E_int = U[4,:,:]
    #     vmag_int = @. u_int^2 + v_int^2
    #     if (USEPLOTPT)
    #         vmagp_int = Vp*vmag_int
    #         scatter(xp,yp,vmagp_int,zcolor=vmagp_int,camera=(0,90))
    #     else
    #         scatter(x,y,vmag_int,zcolor=vmag_int,camera=(0,90))
    #     end
    #     frame(anim)
    #     push!(Uhist,U)
    # end
...
# gif(anim,"fig/dg2D_CNS_quad_convergence/N=$N,K1D=$K1D,T=$T,LIMITOPT=$LIMITOPT,POSDETECT=$POSDETECT,LBOUNDTYPE=$LBOUNDTYPE,BCFLUXTYPE=$BCFLUXTYPE,VISCPENTYPE=$VISCPENTYPE,Re=$Re.gif")
