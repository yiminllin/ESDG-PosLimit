...
const LBOUNDTYPE = 0 # 0 if use POSTOL as lower bound, 1 if use 0.1*loworder
...
VX, VY, EToV = uniform_quad_mesh(K1D,Int64(round(K1D/2)))
@. VX = (VX+1)/2
@. VY = (VY+1)/2/2
...
@inline function get_consP(U,f_x,f_y,t,mapP,Fmask,i,iM,xM,yM,uM,vM,k)
    ...
    if is_wall(i,xM)              # If reflective wall
        rhoP   = U[1,iM,k]
        uP     = -uM
        vP     = vM
        rhouP  = rhoP*uP
...
#while i < 2
    ...
    if (i % GIFINT == 0)
        if (USEPLOTPT)
            rho = U[1,:,:]
            rhop = Vp*rho
            scatter(xp,yp,rhop,zcolor=rhop,camera=(0,90),clims=(0.0,150.0))
        else
            rho = U[1,:,:]
            scatter(x,y,rho,zcolor=rho,camera=(0,90),clims=(0.0,150.0))
        end
        frame(anim)
        push!(Uhist,U)
    end
...
gif(anim,"fig/dg2D_euler_quad_test_modifiedBC/N=$N,K1D=$K1D,T=$T,LIMITOPT=$LIMITOPT,POSDETECT=$POSDETECT,LBOUNDTYPE=$LBOUNDTYPE,BCFLUXTYPE=$BCFLUXTYPE.gif")
...
rhop = Vp*rho
scatter(xp,yp,rhop,zcolor=rhop,camera=(0,90),clims=(0.0,150.0))
savefig("fig/dg2D_euler_quad_test_modifiedBC/N=$N,K1D=$K1D,T=$T,LIMITOPT=$LIMITOPT,POSDETECT=$POSDETECT,LBOUNDTYPE=$LBOUNDTYPE,BCFLUXTYPE=$BCFLUXTYPE.png")
...
df = load("dg2D_euler_quad_test_modifiedBC.jld2","data")
push!(df,(N,K,T,t,CFL,LIMITOPT,POSDETECT,LBOUNDTYPE,BCFLUXTYPE,Uhist,thist))
save("dg2D_euler_quad_test_modifiedBC.jld2","data",df)
