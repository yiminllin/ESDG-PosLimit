...
@inline function is_boundary(i,xM,yM)
    return (is_inflow(i,xM,yM) || is_outflow(i,xM,yM) || is_topflow(i,yM) || is_wall(i,xM,yM))
end
...
@inline function get_consP(UP,U,mapP,Fmask,i,iM,xM,yM,uM,vM,k,tid,t)
...
@inline function get_fluxP(fP,f_x,f_y,mapP,Fmask,i,xM,yM,k,UP,tid,t)
    iP,kP = get_infoP(mapP,Fmask,i,k)
...
@inline function get_vP(VUP,VU,mapP,Fmask,i,iM,xM,yM,k,tid,t)
    iP,kP = get_infoP(mapP,Fmask,i,k)
...
@inline function get_sigmaP(sigmaP,sigma_x,sigma_y,i,iM,xM,yM,k,mapP,Fmask,tid,t)
    iP,kP = get_infoP(mapP,Fmask,i,k)
    sigmaP[1,1,tid] = 0.0
    ...
    elseif (is_inflow(i,xM,yM) || is_outflow(i,xM,yM) || is_topflow(i,yM))
        sigmaP[1,1,tid] = 0.0
        sigmaP[2,1,tid] = sigma_x[2,iM,k]
        sigmaP[3,1,tid] = sigma_x[3,iM,k]
        sigmaP[4,1,tid] = sigma_x[4,iM,k]
...
@inline function get_valP(UP,fP,sigmaP,U,f_x,f_y,sigma_x,sigma_y,mapP,Fmask,i,iM,xM,yM,uM,vM,k,tid,t)
    get_consP(UP,U,mapP,Fmask,i,iM,xM,yM,uM,vM,k,tid,t)
    get_fluxP(fP,f_x,f_y,mapP,Fmask,i,xM,yM,k,UP,tid,t)
    get_sigmaP(sigmaP,sigma_x,sigma_y,i,iM,xM,yM,k,mapP,Fmask,tid,t)
end
...
function compute_sigma(prealloc,ops,geom,t)
    ...
    @batch for k = 1:K
        ...
        for i = 1:Nfp
            ...
            xM = x[iM,k]
            yM = y[iM,k]
            get_vP(VUP,VU,mapP,Fmask,i,iM,xM,yM,k,tid,t)
...
function rhs_IDP!(U,rhsU,t,dtl,prealloc,ops,geom,in_s1)
    ...
    compute_sigma(prealloc,ops,geom,t)
    ...
    @batch for k = 1:K
        ...
        for i = 1:Nfp
            ...
            get_valP(UP,fP,sigmaP,U,f_x,f_y,sigma_x,sigma_y,mapP,Fmask,i,iM,xM,yM,uM,vM,k,tid,t)
            λ = λf_arr[i,k]
...
function construct_vtk_file!(U,Uvtk,pvdfile,xvtk,yvtk,t)
    vtk_grid("$(OUTPUTPATH)/dg2D_CNS_quad_dmr_t=$t",xvtk,yvtk) do vtk
        for k = 1:K
            iK = mod1(k,Int(XLENGTH*K1D))
            jK = div(k-1,Int(XLENGTH*K1D))+1
            for c = 1:Nc
...
pvd = paraview_collection("$(OUTPUTPATH)/dg2D_CNS_quad_dmr.pvd")
...
# df = load("$(OUTPUTPATH)/dg2D_CNS_quad_dmr.jld2","data")
push!(df,(N,K,T,t,CFL,dt0,γ,Re,Pr,VX,VY,EToV,LIMITOPT,POSDETECT,LBOUNDTYPE,BCFLUXTYPE,VISCPENTYPE,POSTOL,SAVEINT,Uhist,thist,dt_hist))
save("$(OUTPUTPATH)/dg2D_CNS_quad_dmr.jld2","data",df)
