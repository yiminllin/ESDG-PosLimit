...
function rhs_IDP!(U,rhsU,t,dt,prealloc,ops,geom,in_s1)
    f_x,f_y,rholog,betalog,U_low,F_low,F_high,F_P,L,wspd_arr,λ_arr,λf_arr,dii_arr,L_hist,rhstest_hist = prealloc
    ...
    @batch for k = 1:K
        for i = 1:Np
            for c = 1:Nc
                rhstest += VU[c,i,k]*M[i]*J*rhsU[c,i,k]
            end
        end
    end
    if rhstest > 0.0
        @show rhstest
        error("positive rhstest")
    end
    append!(rhstest_hist,rhstest)
    @show rhstest

    @show Float64(lsum)/Float64(K)
    append!(L_hist,Float64(lsum)/Float64(K))
...
L_hist   = Float64[]
rhstest_hist = Float64[]

prealloc = (f_x,f_y,rholog,betalog,U_low,F_low,F_high,F_P,L,wspd_arr,λ_arr,λf_arr,dii_arr,L_hist,rhstest_hist)
...
@unpack Vp = rd
gr(aspect_ratio=:equal,legend=false,
   markerstrokewidth=0,markersize=4,ticks=nothing,border=nothing,axis=nothing)
xp,yp,rhop = (x->Vp*x).((x,y,U[1,:,:]))

Plots.scatter(xp[:],yp[:],rhop[:],zcolor=rhop[:],camera=(0,90),c=:haline)
savefig("rho_li.png")
default()
plot(1:length(L_hist),L_hist)
savefig("L_hist_li.png")
plot(1:length(rhstest_hist),rhstest_hist)
savefig("rhstest_hist_li.png")
