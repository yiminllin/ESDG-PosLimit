...
# const rhoL = 3.0
# const pL   = 1.0
# const cL   = sqrt(γ*pL/rhoL)
# const uL   = cL
# const rhoR = 1/2
# const pR   = pL*(rhoR/rhoL)^γ
# const cR   = sqrt(γ*pR/rhoR)
# const uR   = uL+2*(cL)/(γ-1)-2*(cR)/(γ-1)
# T = 0.5
# exact_sol = exact_sol_Rarefraction
        ...
        for i = 1:N+1
            for j = 1:N+1
                if i != j # skip diagonal
                    ...
                    for c = 1:3
                        # fluxS = flux_central(S[i,j],flux[c][i,k],flux[c][j,k])
                        # F_high[c][i,j] = fluxS
