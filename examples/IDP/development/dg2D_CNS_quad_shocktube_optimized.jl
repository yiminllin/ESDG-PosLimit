...
const NUMPLOTPT = 6
const SAVEINT   = 1000
const MESHTYPE  = 0  # 0 - uniform mesh, 1 - refined mesh
const ISHIGHORDER = false
const ISLOWORDER = true
...
@inline function wall_flux(rhoM,uM,vM,pM,nx,ny)
...
function rhs_IDP!(U,rhsU,t,dt,prealloc,ops,geom,in_s1)
    ...
    @batch for k = 1:K
        ...
        for i = 1:Nfp
            ...
            #             ny = -1.0
            #         end
            #         f1s,f2s,f3s,f4s = wall_flux(rhoM,uM,vM,pM,nx,ny)
            #         wfi     = 0.0
            #         viscbc1 = 0.0
            #         viscbc2 = 0.0
            #         viscbc3 = 0.0
            ...
            #             viscbc1 = -BrJ_ii_halved*(sigmax_1_M+sigmax_1_P)
            #             viscbc2 = -BrJ_ii_halved*(sigmax_2_M+sigmax_2_P) + wfi*visc_pen_2
            #             viscbc3 = -BrJ_ii_halved*(sigmax_3_M+sigmax_3_P) + wfi*visc_pen_3
            #             viscbc4 = -BrJ_ii_halved*(sigmax_4_M+sigmax_4_P) + wfi*visc_pen_4
            #         end
            #         if is_face_y(i)
            #             wfi = abs(BsJ_ii_halved)*2
            #             viscbc1 = -BsJ_ii_halved*(sigmay_1_M+sigmay_1_P)
            #             viscbc2 = -BsJ_ii_halved*(sigmay_2_M+sigmay_2_P) + wfi*visc_pen_2
            #             viscbc3 = -BsJ_ii_halved*(sigmay_3_M+sigmay_3_P) + wfi*visc_pen_3
            #             viscbc4 = -BsJ_ii_halved*(sigmay_4_M+sigmay_4_P) + wfi*visc_pen_4
            #         end
            #         F_P[1,i,tid] = wfi*f1s + viscbc1
            #         F_P[2,i,tid] = wfi*f2s + viscbc2
            #         F_P[3,i,tid] = wfi*f3s + viscbc3
            #         F_P[4,i,tid] = wfi*f4s + viscbc4
            #     end
            # end
        end
...
const J_const   = 1.0/K1D/K1D/4
const Jf_const  = 1.0/K1D/2
const rxJ_const = 2*K1D*J_const
const syJ_const = 2*K1D*J_const
const sJ_const  = Jf_const
const rxJ_sq_const = rxJ_const^2
const syJ_sq_const = syJ_const^2
const rxJ_db_const = 2*rxJ_const
const syJ_db_const = 2*syJ_const
...
# Scale by Jacobian
S0xJ_vec   = rxJ_const*S0r_vec
S0yJ_vec   = syJ_const*S0s_vec
SxJ_vec    = rxJ_const*Sr_vec
SyJ_vec    = syJ_const*Ss_vec
SxJ_db_vec = 2*rxJ_const*Sr_vec
SyJ_db_vec = 2*syJ_const*Ss_vec
BrJ_halved = Jf_const*Br_halved
BsJ_halved = Jf_const*Bs_halved
Sr_db_vec = 2*Sr_vec
Ss_db_vec = 2*Ss_vec
...
g_min = minimum(g)
g_max = maximum(g)
vvv = @. exp(-10*(g-g_min)/(g_max-g_min))

xp    = Vp*x
yp    = Vp*y
rhop  = Vp*rho
vvvp  = Vp*vvv
scatter(xp,yp,vvvp,zcolor=vvvp,camera=(0,90))
savefig("fig/dg2D_CNS_quad_shocktube_optimized/Re=$Re,N=$N,K1D=$K1D,T=$T.png")

# open("N=$N,K1D=$K1D,t=$t,CFL=$CFL,rho,shocktube.txt","w") do io
#     writedlm(io,U[1,:,:])
# end
# open("N=$N,K1D=$K1D,t=$t,CFL=$CFL,rhou,shocktube.txt","w") do io
#     writedlm(io,U[2,:,:])
# end
# open("N=$N,K1D=$K1D,t=$t,CFL=$CFL,rhov,shocktube.txt","w") do io
#     writedlm(io,U[3,:,:])
# end
# open("N=$N,K1D=$K1D,t=$t,CFL=$CFL,E,shocktube.txt","w") do io
#     writedlm(io,U[4,:,:])
# end
