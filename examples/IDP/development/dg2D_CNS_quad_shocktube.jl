...
function viscous_matrices(v1,v2,v3,v4)
    λ=-lambda
    μ=mu
    inv_v4_cubed = @. 1/(v4^3)
    λ2μ = (λ+2.0*μ)
    K11_22 = inv_v4_cubed*-λ2μ*v4^2
    K11_24 = inv_v4_cubed*λ2μ*v2*v4
    K11_33 = inv_v4_cubed*-μ*v4^2
    K11_34 = inv_v4_cubed*μ*v3*v4
    K11_42 = inv_v4_cubed*λ2μ*v2*v4
    K11_43 = inv_v4_cubed*μ*v3*v4
    K11_44 = inv_v4_cubed*-(λ2μ*v2^2 + μ*v3^2 - γ*μ*v4/Pr)

    K12_23 = inv_v4_cubed*-λ*v4^2
    K12_24 = inv_v4_cubed*λ*v3*v4
    K12_32 = inv_v4_cubed*-μ*v4^2
    K12_34 = inv_v4_cubed*μ*v2*v4
    K12_42 = inv_v4_cubed*μ*v3*v4
    K12_43 = inv_v4_cubed*λ*v2*v4
    K12_44 = inv_v4_cubed*(λ+μ)*(-v2*v3)

    K22_22 = inv_v4_cubed*-μ*v4^2
    K22_24 = inv_v4_cubed*μ*v2*v4
    K22_33 = inv_v4_cubed*-λ2μ*v4^2
    K22_34 = inv_v4_cubed*λ2μ*v3*v4
    K22_42 = inv_v4_cubed*μ*v2*v4
    K22_43 = inv_v4_cubed*λ2μ*v3*v4
    K22_44 = inv_v4_cubed*-(λ2μ*v3^2 + μ*v2^2 - γ*μ*v4/Pr)
...
function viscous_matrices!(Kxx,Kxy,Kyy,v1,v2,v3,v4)
    lam = -lambda
    μ   = mu
    inv_v4_cubed = @. 1/(v4^3)
    λ2μ = (λ+2.0*μ)
...
@inline function check_BC(xM,yM,i)
    topwall    = ((abs(yM-0.5) < TOL) & (i > TOPRIGHT) & (i <= TOPLEFT))
    leftwall   = ((abs(xM) < TOL) & (i > TOPLEFT))
    rightwall  = ((abs(xM-1.0) < TOL) & (i > BOTTOMRIGHT) & (i <= TOPRIGHT))
    bottomwall = ((abs(yM) < TOL) & (i <= BOTTOMRIGHT))
    # topwall    = ((abs(yM-0.5) < TOL) & (abs(xM) > TOLBC) & (abs(xM-1.0) > TOLBC) & (i > TOPRIGHT) & (i <= TOPLEFT))
    # leftwall   = ((abs(xM) < TOL) & (abs(yM) > TOLBC) & (abs(yM-0.5) > TOLBC) & (i > TOPLEFT))
    # rightwall  = ((abs(xM-1.0) < TOL) & (abs(yM) > TOLBC) & (abs(yM-0.5) > TOLBC) & (i > BOTTOMRIGHT) & (i <= TOPRIGHT))
    # bottomwall = ((abs(yM) < TOL) & (abs(xM) > TOLBC) & (abs(xM-1.0) > TOLBC) & (i <= BOTTOMRIGHT))
    wall     = (leftwall | rightwall | bottomwall)
    has_bc   = (topwall | wall)
    return topwall,leftwall,rightwall,bottomwall,has_bc
end

@inline function get_vP(VU,mapP,Fmask,i,iM,k,xM,t,topwall,leftwall,rightwall,bottomwall,has_bc)
    # if topwall
    #     v1P = VU[1,iM,k]
    #     v2P = VU[2,iM,k]
    #     v3P = -VU[3,iM,k]
    #     v4P = VU[4,iM,k]
    # elseif (leftwall | rightwall | bottomwall)
    #     v1P = VU[1,iM,k]
    #     v2P = -VU[2,iM,k]
    #     v3P = -VU[3,iM,k]
    #     v4P = VU[4,iM,k]
...
@inline function get_consP(U,f_x,f_y,t,mapP,Fmask,i,iM,xM,yM,uM,vM,k,topwall,leftwall,rightwall,bottomwall,has_bc)
    if (topwall | bottomwall)
        rhoP  = U[1,iM,k]
        EP    = U[4,iM,k]
        rhouP = U[2,iM,k]
...
@inline function get_fluxP(U,f_x,f_y,t,mapP,Fmask,i,iM,xM,yM,uM,vM,k,topwall,leftwall,rightwall,bottomwall,has_bc,rhoP,rhouP,rhovP,EP)
...
@inline function get_sigmaP(sigma_x,sigma_y,mapP,Fmask,i,iM,k,topwall,leftwall,rightwall,bottomwall,has_bc)
    # if topwall
    #     sigmax_1_P = 0.0
...
@inline function get_valP(U,f_x,f_y,sigma_x,sigma_y,t,mapP,Fmask,i,iM,xM,yM,uM,vM,k)
    ...
    rhoP,rhouP,rhovP,EP = get_consP(U,f_x,f_y,t,mapP,Fmask,i,iM,xM,yM,uM,vM,k,topwall,leftwall,rightwall,bottomwall,has_bc)
    fx_1_P,fx_2_P,fx_3_P,fx_4_P,fy_1_P,fy_2_P,fy_3_P,fy_4_P = get_fluxP(U,f_x,f_y,t,mapP,Fmask,i,iM,xM,yM,uM,vM,k,topwall,leftwall,rightwall,bottomwall,has_bc,rhoP,rhouP,rhovP,EP)
    sigmax_1_P,sigmax_2_P,sigmax_3_P,sigmax_4_P,sigmay_1_P,sigmay_2_P,sigmay_3_P,sigmay_4_P = get_sigmaP(sigma_x,sigma_y,mapP,Fmask,i,iM,k,topwall,leftwall,rightwall,bottomwall,has_bc)
...
function rhs_IDP!(U,rhsU,t,dt,prealloc,ops,geom,in_s1,istep)
    f_x,f_y,VU,sigma_x,sigma_y,rholog,betalog,U_low,F_low,F_high,F_P,L,wspd_arr,λ_arr,λf_arr,dii_arr = prealloc
    S0r_vec,S0s_vec,S0r_nnzi,S0r_nnzj,S0s_nnzi,S0s_nnzj,Sr_db_vec,Ss_db_vec,Sr_nnzi,Sr_nnzj,Ss_nnzi,Ss_nnzj,Sr_vec,Ss_vec,M_inv,Br_halved,Bs_halved,coeff_arr,S_nnzi,S_nnzj = ops
    ...
    @batch for k = 1:K 
        ...
        for c_r = 1:Sr_nnz_hv
            ...
            j = Sr_nnzj[c_r]
            for c = 1:Nc
                sigma_x[c,i,k] = sigma_x[c,i,k] + rxJ_k*Sr_vec[c_r]*VU[c,j,k]
                sigma_x[c,j,k] = sigma_x[c,j,k] - rxJ_k*Sr_vec[c_r]*VU[c,i,k]
            end
        end
        ...
        for c_s = 1:Ss_nnz_hv
            ...
            j = Ss_nnzj[c_s]
            for c = 1:Nc
                sigma_y[c,i,k] = sigma_y[c,i,k] + syJ_k*Ss_vec[c_s]*VU[c,j,k]
                sigma_y[c,j,k] = sigma_y[c,j,k] - syJ_k*Ss_vec[c_s]*VU[c,i,k]
            end
        end
        ...
        for i = 1:Nfp
            sJ_i = sJ[i,k]
            # if is_face_x(i)
            #     sJ_i = (y[13,k]-y[1,k])/2.0
            # else
            #     sJ_i = (x[4,k]-x[1,k])/2.0
            # end
            iM = Fmask[i]
            xM = x[iM,k]
            yM = y[iM,k]
            ...
            if is_face_x(i)
                BrJ_ii_halved = sJ_i*Br_halved[iM]
            ...
            if is_face_y(i)
                BsJ_ii_halved = sJ_i*Bs_halved[iM]
        ...
        for i = 1:Np
            ...
            K12_23,K12_24,K12_32,K12_34,K12_42,K12_43,K12_44,
            K22_22,K22_24,K22_33,K22_34,K22_44 = viscous_matrices(v1,v2,v3,v4)#get_Kvisc(v1,v2,v3,v4) TODO:
    ...
    @batch for k = 1:K
        ...
        for i = 1:Nfp
            ...
            # end
            iM = Fmask[i]
            BrJ_ii_halved_abs = abs(sJ_i*Br_halved[iM])
            BsJ_ii_halved_abs = abs(sJ_i*Bs_halved[iM])
            xM    = x[iM,k]
            yM    = y[iM,k]
    ...
    if in_s1
        for k = 1:K
            ...
            J_k   = J[1,k]
            for i = 1:Np
                dt = min(dt,1.0/(M_inv[i]/J_k)/2.0/dii_arr[i,k])
            end
        end
    end
    ...
    @batch for k = 1:K
        ...
        for i = 1:Np
            for c = 1:Nc
                U_low[c,i,tid] = U[c,i,k] - dt*(M_inv[i]/J_k)*U_low[c,i,tid]
            end
        end
        ...
        for ni = 1:S_nnz_hv
            i = S_nnzi[ni]
            j = S_nnzj[ni]
            coeff_i = dt*coeff_arr[i]/J_k
            coeff_j = dt*coeff_arr[j]/J_k
            rhoi  = U_low[1,i,tid]
            rhoui = U_low[2,i,tid]
            rhovi = U_low[3,i,tid]
            Ei    = U_low[4,i,tid]
    ...
    @batch for k = 1:K
        ...
        for i = 1:Np
            for c = 1:Nc
                rhsU[c,i,k] = (M_inv[i]/J_k)*rhsU[c,i,k]
            end
        end
    end
...
@unpack xf,yf,mapM,mapP,mapB,nxJ,nyJ,x,y,J,rxJ,syJ,sJ = md
...
ops      = (S0r_vec,  S0s_vec,  S0r_nnzi,S0r_nnzj,S0s_nnzi,S0s_nnzj,
            Sr_db_vec,Ss_db_vec,Sr_nnzi, Sr_nnzj, Ss_nnzi, Ss_nnzj,
            Sr_vec,   Ss_vec,
            M_inv,Br_halved,Bs_halved,coeff_arr,S_nnzi,S_nnzj)
...
open("/data/yl184/N=$N,K1D=$K1D,t=$t,CFL=$CFL,x,dmr.txt","w") do io
    writedlm(io,x)
end
open("/data/yl184/N=$N,K1D=$K1D,t=$t,CFL=$CFL,y,dmr.txt","w") do io
    writedlm(io,y)
end
...
# TODO: Re = 10000, K1D = 40, works for quad_dmr, not quad_shocktube
@time while t < T
# while i < 3
    # SSPRK(3,3)
    fill!(dii_arr,0.0)
    # dt = min(dt0,T-t)
    dt = dt0
    dt = rhs_IDP!(U,rhsU,t,dt,prealloc,ops,geom,true,i);
    dt = min(CFL*dt,T-t)
    @. resW = U + dt*rhsU
    rhs_IDP!(resW,rhsU,t+dt,dt,prealloc,ops,geom,false,i);
    @. resW = resW+dt*rhsU
    @. resW = 3/4*U+1/4*resW
    rhs_IDP!(resW,rhsU,t+dt/2,dt,prealloc,ops,geom,false,i);
    @. resW = resW+dt*rhsU
    @. U = 1/3*U+2/3*resW

    # # TODO: hard fix corner
    # U[1,1,1] = rhoL
    # U[2,1,1] = rhouL
    # U[3,1,1] = rhovL
    # U[4,1,1] = EL
    # U[1,3*(N+1)+1,K-2*K1D+1] = rhoL
    # U[2,3*(N+1)+1,K-2*K1D+1] = rhouL
    # U[3,3*(N+1)+1,K-2*K1D+1] = rhovL
    # U[4,3*(N+1)+1,K-2*K1D+1] = EL
    # U[1,Np,K] = rhoR
    # U[2,Np,K] = rhouR
    # U[3,Np,K] = rhovR
    # U[4,Np,K] = ER
    ...
    if (mod(i,250) == 1)
    # if i >= 480
        open("/data/yl184/N=$N,K1D=$K1D,t=$t,CFL=$CFL,rho,dmr.txt","w") do io
            writedlm(io,U[1,:,:])
        end
        open("/data/yl184/N=$N,K1D=$K1D,t=$t,CFL=$CFL,rhou,dmr.txt","w") do io
            writedlm(io,U[2,:,:])
        end
        open("/data/yl184/N=$N,K1D=$K1D,t=$t,CFL=$CFL,rhov,dmr.txt","w") do io
            writedlm(io,U[3,:,:])
        end
        open("/data/yl184/N=$N,K1D=$K1D,t=$t,CFL=$CFL,E,dmr.txt","w") do io
            writedlm(io,U[4,:,:])
        end
        ...
        open("/data/yl184/N=$N,K1D=$K1D,t=$t,CFL=$CFL,gradg,dmr.txt","w") do io
            writedlm(io,vv)
        end

        # open("/data/yl184/N=$N,K1D=$K1D,t=$t,CFL=$CFL,sigmax,dmr.txt","w") do io
        #     writedlm(io,sigma_x)
        # end
        # open("/data/yl184/N=$N,K1D=$K1D,t=$t,CFL=$CFL,sigmay,dmr.txt","w") do io
        #     writedlm(io,sigma_y)
        # end
