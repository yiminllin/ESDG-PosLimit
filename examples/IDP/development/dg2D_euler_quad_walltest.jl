...
@inline function get_valP(U,f_x,f_y,mapP,Fmask,i,iM,xM,uM,vM,k)
...
function rhs_IDP!(U,rhsU,t,dt,prealloc,ops,geom,in_s1)
    ...
    @batch for k = 1:K
        for i = 1:Nfp
            ...
            rhoP,rhouP,rhovP,EP,_,_,_,_,_,_,_,_,has_bc = get_valP(U,f_x,f_y,mapP,Fmask,i,iM,xM,uM,vM,k)
            iP,kP = get_infoP(mapP,Fmask,i,k)
    ...
    @batch for k = 1:K
        ...
        for i = 1:Nfp
            ...
            rhoP,rhouP,rhovP,EP,fx_1_P,fx_2_P,fx_3_P,fx_4_P,fy_1_P,fy_2_P,fy_3_P,fy_4_P,_ = get_valP(U,f_x,f_y,mapP,Fmask,i,iM,xM,uM,vM,k)
            λ = λf_arr[i,k]
...
savefig("~/Desktop/N=$N,K1D=$K1D,T=$T,euler_reflectivewall.png")
