...
mapN = collect(reshape(1:Np*K,Np,K))
wall_nodal = mapN[findall(@. (abs(x) < TOL) | (abs(x-1.0) < TOL) | (abs(y) < TOL))]

@inline function enforce_BC_timestep!(U,wall_nodal)
    for i = wall_nodal
        k = fld1(i,Np)
        j = mod1(i,Np)
        U[2,j,k] = 0.0
...
open("/data/yl184/N=$N,K1D=$K1D,t=$t,CFL=$CFL,x=$XLENGTH,x,shocktube.txt","w") do io
    writedlm(io,x)
end
open("/data/yl184/N=$N,K1D=$K1D,t=$t,CFL=$CFL,x=$XLENGTH,y,shocktube.txt","w") do io
    writedlm(io,y)
end
...
#while i < 2
    ...
    if (mod(i,400) == 1)
    # if i >= 480
        open("/data/yl184/N=$N,K1D=$K1D,t=$t,CFL=$CFL,rho,shocktube.txt","w") do io
            writedlm(io,U[1,:,:])
        end
        open("/data/yl184/N=$N,K1D=$K1D,t=$t,CFL=$CFL,rhou,shocktube.txt","w") do io
            writedlm(io,U[2,:,:])
        end
        open("/data/yl184/N=$N,K1D=$K1D,t=$t,CFL=$CFL,rhov,shocktube.txt","w") do io
            writedlm(io,U[3,:,:])
        end
        open("/data/yl184/N=$N,K1D=$K1D,t=$t,CFL=$CFL,E,shocktube.txt","w") do io
            writedlm(io,U[4,:,:])
        end
        ...
        open("/data/yl184/N=$N,K1D=$K1D,t=$t,CFL=$CFL,gradg,shocktube.txt","w") do io
            writedlm(io,vv)
        end

        # open("/data/yl184/N=$N,K1D=$K1D,t=$t,CFL=$CFL,x=$XLENGTH,sigmax,shocktube.txt","w") do io
        #     writedlm(io,sigma_x)
        # end
