...
function rhs_low_viscous(U,K,N,Qr)
    J = (Br-Bl)/K/2 # assume uniform interval
    Nc = 3
    rhsU  = [zeros(N+1,K) for i = 1:Nc]
    theta = [zeros(N+1,K) for i = 1:Nc]
...
function rhs_low_graph_visc(U,K,N,sigma,S0,S)
    ...
    for k = 1:K
        for j = 1:N+1
            for i = 1:N+1
                if i != j
                    F1 = S0[i,j]*(flux[1][i,k]+flux[1][j,k])-S[i,j]*(sigma[1][i,k]+sigma[1][j,k])
                    F2 = S0[i,j]*(flux[2][i,k]+flux[2][j,k])-S[i,j]*(sigma[2][i,k]+sigma[2][j,k])
                    F3 = S0[i,j]*(flux[3][i,k]+flux[3][j,k])-S[i,j]*(sigma[3][i,k]+sigma[3][j,k])
...
function rhs_low(U,K,N,Mlump_inv,S0,S,Qr0,Qr)
    J = (Br-Bl)/K/2 # assume uniform interval
    Nc = 3
    rhsU = [zeros(N+1,K) for i = 1:Nc]
    rhsI = rhs_low_inviscid(U,K,N,S0)
    rhsV,sigma = rhs_low_viscous(U,K,N,Qr)
...
@gif for i = 1:Nsteps
    ...
    rhsU = rhs_low(U,K,N,Mlump_inv,S0,S,Qr0,Qr)
    @. U = U + dt*rhsU
    global t = t + dt
