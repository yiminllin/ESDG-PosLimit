...
function rhs_IDP_fixdt!(U,N,K1D,Minv,Vf,Dr,Ds,nxJ,nyJ,Sr,Ss,S0r,S0s,S0r1,S0s1,LIFT,mapP,face_idx,x_idx,y_idx,boundary,nx_b,ny_b,topwall,nx_top,ny_top,wall,nx_wall,ny_wall,dt,VU,flux_x,flux_y,lam,LFc,Ub,sigma_x,sigma_y,J,rxJ,syJ,sJ)
    # TODO: hardcoded!
    K      = size(U[1],2)
    Np     = (N+1)*(N+1)
    Nfaces = 4
    ...
    VUr = (x->Dr*x).(VU)
    VUs = (x->Ds*x).(VU)
    VUx = volx.(VUr) .+ surfx.(VUP,VUf)
    VUy = voly.(VUs) .+ surfy.(VUP,VUf)
    # VUx = (x->x./J).(VUx)
    # VUy = (x->x./J).(VUy)
    for c = 1:Nc
        @. VUx[c] = VUx[c]/J
    ...
    Threads.@threads for k = 1:K
        ...
        for i = 1:Np
            for j = 1:Np
                c_ij_norm = sqrt(rxJ_k^2*S0r[i,j]^2+syJ_k^2*S0s[i,j]^2)
                if abs(c_ij_norm) >= TOL
                    n_ij_x = rxJ_k*S0r[i,j]/c_ij_norm
                    n_ij_y = syJ_k*S0s[i,j]/c_ij_norm
                    ...
                    for c = 1:Nc
                        F_low[c][i,j,tid] = (rxJ_k*S0r[i,j]*(flux_x[c][i,k]+flux_x[c][j,k])
                                            +syJ_k*S0s[i,j]*(flux_y[c][i,k]+flux_y[c][j,k])
                                            -rxJ_k*S0r[i,j]*(sigma_x[c][i,k]+sigma_x[c][j,k])
                                            -syJ_k*S0s[i,j]*(sigma_y[c][i,k]+sigma_y[c][j,k])
        ...
        for i = 1:Np-1
            for j = i+1:Np
                ...
                if Sr[i,j] != 0.0 || Ss[i,j] != 0.0
                    ...
                    for c = 1:Nc
                        val = (2*rxJ_k*Sr[i,j]*F1[c]+2*syJ_k*Ss[i,j]*F2[c]
                                -rxJ_k*S0r[i,j]*(sigma_x[c][i,k]+sigma_x[c][j,k])
                                -syJ_k*S0s[i,j]*(sigma_y[c][i,k]+sigma_y[c][j,k]))
        ...
        for i = 1:Np
            ...
            for j = 1:Np
                if i != j
                    ...
                    L[i,j,tid] = limiting_param(U_low_i[:,tid], P_ij[:,tid])
                end
            end
        end
    ...
    # for k = 1:K
    #     for c = 1:Nc
    #         rhsU[c][:,k] = 1/J[1,k]*Minv*rhsU[c][:,k]
    #     end
    # end
    for k = 1:K
        J_k = J[1,k]
...
function rhs_IDP_vardt!(U,N,K1D,M,Minv,Vf,Dr,Ds,nxJ,nyJ,Sr,Ss,S0r,S0s,S0r1,S0s1,LIFT,mapP,face_idx,x_idx,y_idx,boundary,nx_b,ny_b,topwall,nx_top,ny_top,wall,nx_wall,ny_wall,VU,flux_x,flux_y,lam,LFc,Ub,sigma_x,sigma_y,J,rxJ,syJ,sJ)
    # TODO: hardcoded!
    K      = size(U[1],2)
    Np     = (N+1)*(N+1)
    Nfaces = 4
    ...
    for k = 1:K
        for i = 1:Np
            J_k = J[i,k] 
            dt = min(-.5*J_k*M[i,i]/d_ii_arr[i,k],dt)
        end
    end
    #dt = minimum(-J/2*M*(1 ./d_ii_arr))

    return rhs_IDP_fixdt!(U,N,K1D,Minv,Vf,Dr,Ds,nxJ,nyJ,Sr,Ss,S0r,S0s,S0r1,S0s1,LIFT,mapP,face_idx,x_idx,y_idx,boundary,nx_b,ny_b,topwall,nx_top,ny_top,wall,nx_wall,ny_wall,dt,VU,flux_x,flux_y,lam,LFc,Ub,sigma_x,sigma_y,J,rxJ,syJ,sJ), dt
end
...
while t < T
    ...
    rhsU,dt = rhs_IDP_vardt!(U,N,K1D,M,Minv,Vf,Dr,Ds,nxJ,nyJ,Sr,Ss,S0r,S0s,S0r1,S0s1,LIFT,mapP,face_idx,x_idx,y_idx,boundary,nx_b,ny_b,topwall,nx_top,ny_top,wall,nx_wall,ny_wall,VU,flux_x,flux_y,lam,LFc,Ub,sigma_x,sigma_y,J,rxJ,syJ,sJ);
    dt = min(500*dt,T-t)
    @. resW = U + dt*rhsU
    rhsU = rhs_IDP_fixdt!(resW,N,K1D,Minv,Vf,Dr,Ds,nxJ,nyJ,Sr,Ss,S0r,S0s,S0r1,S0s1,LIFT,mapP,face_idx,x_idx,y_idx,boundary,nx_b,ny_b,topwall,nx_top,ny_top,wall,nx_wall,ny_wall,dt,VU,flux_x,flux_y,lam,LFc,Ub,sigma_x,sigma_y,J,rxJ,syJ,sJ);
    @. resZ = resW+dt*rhsU
    @. resW = 3/4*U+1/4*resZ
    rhsU = rhs_IDP_fixdt!(resW,N,K1D,Minv,Vf,Dr,Ds,nxJ,nyJ,Sr,Ss,S0r,S0s,S0r1,S0s1,LIFT,mapP,face_idx,x_idx,y_idx,boundary,nx_b,ny_b,topwall,nx_top,ny_top,wall,nx_wall,ny_wall,dt,VU,flux_x,flux_y,lam,LFc,Ub,sigma_x,sigma_y,J,rxJ,syJ,sJ);
    @. resZ = resW+dt*rhsU
...
#vv = Vp*U[1]
vv = @. exp(-10*(g-g_min)/(g_max-g_min))
color_schlieren = cgrad(:Blues_9,20,categorical=true,rev=true)
scatter(xp,yp,vv,zcolor=vv,camera=(0,90),ms=1,colorbar=:right,color=color_schlieren)
